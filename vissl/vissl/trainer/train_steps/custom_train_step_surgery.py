...
def vis_samples(sample):
    ...
    imgs = [
        ((im.cpu().squeeze().numpy().transpose(1, 2, 0)) * 255.0).astype(np.uint8)
        for im in images
    ]
    ...
    im_grid = np.zeros(
        (
            img_h * num_rows + margin * (num_rows - 1),
            img_w * num_cols + margin * (num_cols - 1),
            img_c,
        ),
        np.uint8,
    )
    im_grid.fill(0)
    positions = itertools.product(range(num_rows), range(num_cols))
    for (y_i, x_i), img in zip(positions, imgs):
        x = x_i * (img_w + margin)
        y = y_i * (img_h + margin)
        im_grid[y : y + img_h, x : x + img_w, :] = img
...
@register_train_step("custom_train_step_surgery")
def custom_train_step_surgery(task):
    """
